...
void main()
{
...
#if defined(SHADOWS)
	var_pos.xyz = vec3(dot(pos_t, proj_shadow[0]), dot(pos_t, proj_shadow[1]), dot(pos_t, proj_shadow[2]));
#endif
	var_tex = vec4(in_tex.xy, in_mip.zw) * dim_tex;
	var_loc = in_loc;
	...
	float light = (lca - ambient.x) * ambient.y + ambient.x; // rebalanced light intensity
